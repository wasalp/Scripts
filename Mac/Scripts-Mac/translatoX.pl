...
# Make a file that will send instructions to Clustal with appropriate name of input file
	...
	if ($pauser eq "clustalw") {
		...
		$muscleoutfile=$out_file . '.aa_ali.fasta';
		open(LOG, ">$out_file.$pauser.log") || die "Cannot create file outfile.pauser.log: $out_file.$pauser.log\n";
		my $pid = open(PH, "$path"."~/Desktop/muscle -in $muscleinfile -out $muscleoutfile 2>&1 |");              # or with an open pipe
		while(<PH>) {
			print STDERR $_;
			print LOG $_;
		}
...
if($gblocks_params ne "") {
	...
	print H "Gblocks in progress\n";
	system "$path"."~/Desktop/Gblocks $aa_ali_file $gblocks_params";
	system "$path"."~/Desktop/Gblocks $aa_ali_file $gblocks_params -p=s";
